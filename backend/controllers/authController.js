...
const activateSupplierAccount = async ({
  invitationToken,
  nom,
  prenom,
  email,
  ...
  const supplierResult = await pool.query(
    `SELECT u.*, f.statut_inscription, f.expiration_token
         FROM utilisateur u
         JOIN fournisseur f ON f.id_utilisateur = u.id
         WHERE f.token_invitation = $1`,
    [invitationToken],
  );
  ...
  if (supplierResult.rows.length === 0) {
    return {
      status: 404,
      body: { message: "Invitation invalide ou expirée." },
    };
  }

  const user = supplierResult.rows[0];
  if (user.est_actif === true || user.statut_inscription === "VALIDE") {
    return { status: 400, body: { message: "Ce compte est déjà activé." } };
  }

  if (user.expiration_token && new Date() > user.expiration_token) {
    return {
      status: 400,
      body: { message: "Le token d'invitation a expiré." },
    };
  }
  ...
  if (email && email !== user.email) {
    return {
      status: 400,
      body: { message: "L'email ne correspond pas à l'invitation." },
    };
  }
  if (nom && nom !== user.nom) {
    return {
      status: 400,
      body: { message: "Le nom ne correspond pas à l'invitation." },
    };
  }
  if (prenom && prenom !== user.prenom) {
    return {
      status: 400,
      body: { message: "Le prénom ne correspond pas à l'invitation." },
    };
  }

  const accountPassword = password || Math.random().toString(36).slice(-8);
  ...
  try {
    ...
    await client.query(
      `UPDATE fournisseur
             SET statut_inscription = 'VALIDE', token_invitation = NULL, expiration_token = NULL
  ...
  return {
    status: 200,
    body: { message: "Compte fournisseur activé avec succès." },
  };
};

const activateStandardAccount = async ({ nom, prenom, email }) => {
  ...
  if (userResult.rows.length === 0) {
    return {
      ...
      body: {
        message:
          "Les informations saisies ne correspondent à aucun compte pré-enregistré.",
      },
    };
  }
  ...
  await sendActivationEmail(email, prenom, user.role, tempPassword, false);
  return {
    status: 200,
    body: { message: "Succès ! Le mot de passe a été envoyé à votre email." },
  };
};

exports.activateUser = async (req, res) => {
  const { invitationToken, nom, prenom, email, password } = req.body;

  try {
    const result = invitationToken
      ? await activateSupplierAccount({
          invitationToken,
          nom,
          prenom,
          email,
      ...
      : await activateStandardAccount({ nom, prenom, email });
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    await sendTwoFactorCodeEmail(user.email, code2FA);
    ...
    res
      .status(500)
      .json({ error: "Erreur lors du login.", detail: err.message });
  }
};
...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de l'envoi du code.", detail: err.message });
  }
};
...
exports.updateProfile = async (req, res) => {
  ...
  try {
    ...
    await logAction(
      userId,
      "UPDATE_PROFIL",
      `Modification profil : ${prenom} ${nom}`,
    );
  } catch (err) {
    console.error("❌ updateProfile error:", err.message);
    res
      .status(500)
      .json({ error: "Erreur de mise à jour.", detail: err.message });
  }
};
...
exports.changePassword = async (req, res) => {
  ...
  try {
    ...
    if (!passwordRegex.test(newPassword)) {
      return res.status(400).json({
        error:
          "Mot de passe trop faible. Il doit contenir 8 caractères minimum, une majuscule, un chiffre et un caractère spécial.",
      });
    }

    const result = await pool.query(
      "SELECT password_hash FROM utilisateur WHERE id = $1",
      [userId],
    );
    if (result.rows.length === 0) {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors du changement.", detail: err.message });
  }
};
...
exports.uploadAvatar = async (req, res) => {
  ...
  try {
    ...
    await pool.query("UPDATE utilisateur SET avatar = $1 WHERE id = $2", [
      base64,
      userId,
    ]);
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de l'upload.", detail: err.message });
  }
};
