...
exports.getUsers = async (req, res) => {
  ...
  try {
    ...
    if (userRole === "COMMERCIAL") {
      ...
      return res
        .status(403)
        .json({ message: "Accès non autorisé à la liste." });
    }
...
exports.getFournisseurs = async (req, res) => {
  try {
    ...
    await logAction(
      req.user.id,
      "VIEW_FOURNISSEURS",
      "Consultation liste fournisseurs",
    );
...
exports.createUser = async (req, res) => {
  ...
  if (role === "CLIENT") {
    return res.status(400).json({
      message:
        "La création d'un client doit passer par le flux de validation des demandes d'adhésion.",
    });
  }
  ...
  if (role === "ADMIN") {
    return res.status(403).json({
      message:
        "La création d'un compte administrateur est interdite via cette interface.",
    });
  }
  ...
  if (role === "COMMERCIAL" && (!region || !num_tlp)) {
    return res.status(400).json({
      message:
        "La région est obligatoire pour la création d'un compte commercial.",
    });
  }

  // Pour FOURNISSEUR, utiliser la méthode par invitation
  if (role === "FOURNISSEUR") {
    ...
    try {
      ...
      const expirationToken = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 jours
      ...
      // Créer l'entrée fournisseur avec le token
      await db.query(
      ...
      await sendInvitationEmail(email, nom, prenom, role, invitationToken);
      ...
      return res.status(200).json({
        message: `Invitation envoyée à ${prenom} ${nom} pour le rôle ${role}.`,
      });
    } catch (err) {
      ...
      return res
        .status(500)
        .json({ error: "Erreur lors de l'invitation du fournisseur." });
    } finally {
      db.release();
    }
  }
  ...
  try {
    ...
    if (currentRole === "COMMERCIAL") {
      if (!region) {
        ...
        return res
          .status(400)
          .json({ message: "La région est obligatoire pour un commercial." });
      }
      await db.query(
        `UPDATE commercial 
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    ...
    if (role === "ADMIN")
      return res
        .status(403)
        .json({ message: "Suppression d'un administrateur interdite." });
    ...
    if (role === "COMMERCIAL") {
      ...
      if (clients.rows.length > 0) {
        if (!nouveau_commercial_id)
          return res.status(400).json({
            message:
              "Ce commercial a des clients. Veuillez fournir un nouveau_commercial_id.",
          });
        ...
        if (newComCheck.rows.length === 0)
          return res
            .status(400)
            .json({ message: "Nouveau commercial introuvable ou invalide." });
